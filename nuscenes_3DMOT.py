...
def getRotation(coords):
    diffs = [coords[1] - coords[7], coords[0] - coords[6]]
    rotation = np.arctan(diffs[0] / diffs[1]) * 180 / np.pi
    if (diffs[1] < 0):
        rotation += 180
    if (diffs[0] < 0):
        rotation += 360

    return np.pi/180*rotation
...
def track_one_seq(seq_id,config,nusc,my_scene):
    ...
    for i in range(nbr_samples):
        ...
        start = time.time()
        # nuscenes_data['det_scores'][nuscenes_data['det_scores'] > 0] = 1.0
        ...
        for i in range(nbr_samples):
            ...
            if i in frame_first_dict.keys():
                ...
                for ob_id in objects.keys():
                    ...
                    box_angle = getRotation([transformed_corners_lidar[0, 0], transformed_corners_lidar[1, 0],
                                 transformed_corners_lidar[0, 4], transformed_corners_lidar[1, 4],
                                 transformed_corners_lidar[0, 5], transformed_corners_lidar[1, 5],
                                 transformed_corners_lidar[0, 1], transformed_corners_lidar[1, 1]])
