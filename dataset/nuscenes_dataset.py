...
class nuscenesTrackingDataset:
    ...
    def __getitem__(self, input_sample):
        ...
        pose = transform_matrix(ego_pose['translation'], Quaternion(ego_pose['rotation']),
                                inverse=True)
        pose_angle = Quaternion(ego_pose['rotation']).inverse.yaw_pitch_roll[0]
        pose_angle_chk = Quaternion(ego_pose['rotation']).yaw_pitch_roll[0]
